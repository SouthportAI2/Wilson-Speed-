...
const AudioLogger: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto lg:min-h-[600px] animate-fade-in">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-black uppercase tracking-widest text-slate-400">
                {isConnected ? 'Node Connected' : 'Connection Error'}
              </span>
            </div>
        ...
        <div className="flex-1 bg-slate-800/20 rounded-3xl border border-slate-700/30 overflow-y-auto p-4 space-y-3 min-h-[400px]">
          ...
          {filteredLogs.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full py-20 text-slate-500 space-y-4">
              ...
              <p className="font-medium text-slate-600">No matching infrastructure records.</p>
            </div>
          )}
        </div>
      </div>
          ...
          {selectedLog ? (
            <div className="space-y-8 animate-fade-in">
              ...
              <div className="bg-slate-950/50 p-6 rounded-2xl border border-slate-800 min-h-[200px]">
                ...
                <p className="text-slate-300 text-sm leading-relaxed font-mono whitespace-pre-wrap">{selectedLog.transcriptPreview}</p>
              </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-20">
              <FileText size={64} />
              <p className="font-bold uppercase tracking-[0.2em] text-xs">Select a data node to begin</p>
            </div>
          )}
        </div>
        ...
        <div className="p-6 bg-slate-950/30 border-t border-slate-800">
          <form onSubmit={handleAssistantAsk} className="relative">
            <input 
              ...
              className="w-full bg-slate-900/80 border border-slate-700 rounded-xl pl-4 pr-12 py-4 text-sm text-white focus:ring-1 focus:ring-blue-500 outline-none transition-all"
            />
